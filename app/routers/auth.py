...
from app.schemas.user import UserCreate, UserLogin, UserUpdate, UserResponse,UserAdminResponse, PasswordUpdate
...
def registrar(user_data: UserCreate,request: Request, db: Session = Depends(get_db)):
    ip = obtener_ip_cliente(request)
    return auth_service.registrar_usuario(db, user_data,ip)

@router.post("/login", response_model=Token)
def login(user_data: UserLogin,request: Request, db: Session = Depends(get_db)):
    ip = obtener_ip_cliente(request)
    return auth_service.login_usuario(db, user_data.correo, user_data.password,ip)

@router.post("/logout")
def logout(request: Request,
           db: Session = Depends(get_db),
           current_user: User = Depends(get_current_user)  # ← requiere estar autenticado 
           ):
     ip = obtener_ip_cliente(request)
     return auth_service.logout_usuario(db, current_user,ip)
